...
function loadDefaults(registry) {
	const location = path.join(__dirname, '../modules');
...
function registerModuleInitializer(registry, folder, name) {
	const location = path.normalize(folder + '/' + name + '/initializer.js');

	if(!fs.existsSync(location)) {
		return;
	}

	const initializer = require(location);
...
function loadModuleFactory(location, name) {
	const proto = require(location + '/' + name);
	let factory;
