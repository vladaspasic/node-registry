...
const _ = require('lodash'),
	...
	Holder = require('../container/holder'),
...
const Registry = EventEmitter.extend({
	...
	runInitializers: function(callback) {
		...
		// this.registerInitializer({name: Math.random() + '', initializer(s, e, c) {c()}});

		const tasks = this.__initializers.map((initializer) => {
			return function(cb) {
				initializer.initializer(container, server, cb);
			};
		}, this);

		return utils.series(tasks, (err) => {
			console.log('Initializers run', err);
