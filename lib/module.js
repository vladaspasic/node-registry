...
function loadConfiguration(module, configuration) {
	...
	var path = _.result(module, 'configurationPath');

	if(!_.isEmpty(path)) {
		var config = utils.loadConfigurationFile(path, module.getName());
...
/**
 * Declare a Module Type, it can be 'module' or 'confguration'. Default is 'module';
 *
 * @property type
 * @type {String}
 * @default module
 ...
 * @type {String}
 * @default {process.cwd}/config/{module.name}
 **/
Module.prototype.configurationPath = function() {
	return process.cwd() + '/config/' + this.getName();
};
...
/**
 * Returns the Module configuration, or if the property argument is specified
 * it will return the property value for that name/path.
 *
 * @method getConfiguration
 * @param  {String} property      Property name which to fetch
 * @return {Object} configuration
 */
Module.prototype.getConfiguration = function(property) {
	if(_.isEmpty(property)) return this.configuration;

	return utils.get(this.configuration, property);
};
