...
Registry.prototype.scanDirectories = function(locations) {
	...
	var union = _.union(this._locations, locations);

	utils.defineProperty(this, '_locations', union);
};
...
Registry.prototype.reopen = function(name, callback) {
	...
	if(!module) return callback(new Error('No module with name: ' + name + ' found'));

	try {
		return loader.executeModuleInitialization(module, function(err, data) {
			if(err) return callback(err);

			utils.defineProperty(registry._values, name, data.value);
...
Registry.prototype.start = function(options, application, callback) {
	...
	loader.load(this, function(error, modules) {
